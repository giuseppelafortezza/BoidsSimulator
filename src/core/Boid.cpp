...
{
    float neighbordist = 100; // Field of vision
...
Pvector Boid::Attractive(vector<Boid> Targets)
{
	Pvector sum(0, 0);
	int count = 0;
	for(unsigned int i = 0; i < Targets.size(); i++) 
	{
		float d = location.distance(Targets[i].location);
		sum.addVector(Targets[i].location);
		sum.divScalar(d);
		count++;
	}
	if(count > 0) 
	{
		...
		std::cout << sqrt(sum.x*sum.x + sum.y*sum.y ) << std::endl;
		return seek(sum);
	}
	else
	{
...
Pvector Boid::Repulsive(vector<Boid> Obstacles)
{
	// Distance of field of vision for separation between boids
	float desiredseparation = 60;	//dimension of the obstacles
	Pvector steer(0, 0);
	int count = 0;
	// For every boid in the system, check if it's too close
	...
	{
		// Calculate distance from current boid to boid we're looking at
		float d = location.distance(Obstacles[i].location);
		// If this is a fellow boid and it's too close, move away from it
		if((d > 0) && (d < desiredseparation))
		{
			Pvector diff(0,0);
			diff = diff.subTwoVector(location, Obstacles[i].location);
			diff.normalize();
			diff.divScalar(d);      // Weight by distance
			steer.addVector(diff);
			count++;
...
void Boid::run(vector <Boid> b, vector <Boid> o, vector <Boid> t)
{
    flock(b, o, t);
    update();
    borders();
...
void Boid::flock(vector<Boid> b, vector <Boid> o, vector <Boid> t)
{
    Pvector sep = Separation(b);
    Pvector ali = Alignment(b);
    Pvector coh = Cohesion(b);
	Pvector att = Attractive(t);
	Pvector rep = Repulsive(o);

    sep.mulScalar(kSeparation);
    ali.mulScalar(kAlignment);
    coh.mulScalar(kCohesion);
	att.mulScalar(kAttractive);
	rep.mulScalar(kRepulsive);
...
void Boid::setForceGains(double separ, double align, double cohes)
{
	kSeparation = separ;
	kAlignment = align;
	kCohesion = cohes;
